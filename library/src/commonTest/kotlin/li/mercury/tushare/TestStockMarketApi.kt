...
class TestStockMarketApi : TestTuShare() {
    ...
    @Test
    fun testWeeklyWorks() = test {
        val config = createConfigWithMockEngine("weekly.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getWeekly(
            WeeklyParams(
                tsCode = TsCode("000001", "SZ"),
    ...
    @Test
    fun testMonthlyWorks() = test {
        val config = createConfigWithMockEngine("monthly.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getMonthly(
            MonthlyParams(
                tsCode = TsCode("000001", "SZ"),
    ...
    @Test
    fun testAdjFactorWorks() = test {
        val config = createConfigWithMockEngine("adj_factor.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getAdjFactor(
            AdjFactorParams(
                tsCode = TsCode("000001", "SZ"),
                tradeDate = LocalDate(2018, 7, 16)
            )
    ...
    @Test
    fun testDailyBasicWorks() = test {
        val config = createConfigWithMockEngine("daily_basic.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getDailyBasic(
            DailyBasicParams(
                tsCode = TsCode("000001", "SZ"),
                tradeDate = LocalDate(2018, 7, 16)
            )
    ...
    @Test
    fun testStkLimitWorks() = test {
        val config = createConfigWithMockEngine("stk_limit.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getStkLimit(
            StkLimitParams(
                tsCode = TsCode("000001", "SZ"),
                tradeDate = LocalDate(2018, 7, 16)
            )
    ...
    @Test
    fun testBakDailyWorks() = test {
        val config = createConfigWithMockEngine("bak_daily.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getBakDaily(
            BakDailyParams(
                tsCode = TsCode("000001", "SZ"),
                tradeDate = LocalDate(2018, 7, 16)
            )
    ...
    @Test
    fun testHsgtTop10Works() = test {
        val config = createConfigWithMockEngine("hsgt_top10.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getHsgtTop10(
            HsgtTop10Params(
                tradeDate = LocalDate(2018, 7, 16)
            )
        )
    ...
    @Test
    fun testGgtTop10Works() = test {
        val config = createConfigWithMockEngine("ggt_top10.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getGgtTop10(
            GgtTop10Params(
                tradeDate = LocalDate(2018, 7, 16)
            )
        )
