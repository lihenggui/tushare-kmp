...
class TestException : TestTuShare() {
    ...
    fun apiError() = runTest {
        ...
        // 验证错误信息包含API错误代码
        assertTrue(
            exception.error.detail?.code?.startsWith("api_error_") == true,
            "Error code should start with 'api_error_', but was: ${exception.error.detail?.code}"
        )

        // 验证错误信息不为空
        assertTrue(
            !exception.error.detail.msg.isNullOrBlank(),
            "Error message should not be empty"
        )

        // 验证错误信息包含预期内容
        assertTrue(
            exception.error.detail.msg.contains("invalid api key"),
            "Error message should contain 'invalid api key', but was: ${exception.error.detail.msg}"
        )
    }

    @Test
    fun permissionError() = runTest {
        ...
        // 验证错误信息包含中文权限关键词
        assertTrue(
            exception.error.detail.msg.contains("权限"),
