...
class TestStockBasicApi : TestTuShare() {
    ...
    @Test
    fun testNameChangeWorks() = test {
        val config = createConfigWithMockEngine("name_change.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getNameChange(
            NameChangeParams(
    ...
    @Test
    fun testStockCompanyWorks() = test {
        val config = createConfigWithMockEngine("stock_company.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getStockCompany(
            StockCompanyParams(
    ...
    // @Test
    fun testStkPremarketWorks() = test {
        val config = createConfigWithMockEngine("stk_premarket.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getStkPremarket(
            StkPremarketParams(
    ...
    // Test skipped, no permission
    // @Test
    fun testTradeCalWorks() = test {
        val config = createConfigWithMockEngine("trade_cal.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getTradeCal(
            TradeCalParams(
    ...
    // Test skipped, no permission
    // @Test
    fun testStkManagersWorks() = test {
        val config = createConfigWithMockEngine("stk_managers.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getStkManagers(
            StkManagersParams(
    ...
    // Test skipped, no permission
    // @Test
    fun testStkRewardsWorks() = test {
        val config = createConfigWithMockEngine("stk_rewards.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getStkRewards(
            StkRewardsParams(
                tsCode = TsCode("000001", "SZ"),
            )
        )
    ...
    @Test
    fun testNewShareWorks() = test {
        val config = createConfigWithMockEngine("new_share.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getNewShare(
            NewShareParams(
    ...
    @Test
    fun testBakBasicWorks() = test {
        val config = createConfigWithMockEngine("bak_basic.json")
        val tuShareInstance = generateTuShare(config)

        val result = tuShareInstance.getBakBasic(
            BakBasicParams(
