...
object DataClassConversions {
    ...
    fun snakeToCamelCase(input: String): String {
        return input.replace(Regex("_([a-z])")) { it.groupValues[1].uppercase() }
    }
     ...
     * @return 转换后的Map
     */
    inline fun <reified T> toMap(instance: T, snakeCase: Boolean = true): Map<String, String> {
 ...
 */
inline fun <reified T> T.toApiParams(snakeCase: Boolean = true): Map<String, String> {
    return DataClassConversions.toMap(this, snakeCase)
} 